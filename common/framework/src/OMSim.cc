...
void OMSim::setGeneralOptions()
{
    ...
    ("environment", po::value<G4int>()->default_value(0), "medium in which the setup is emmersed [AIR = 0, ice = 1, spice = 2,WATER=3]")
    ...
    ("single_hemisphere", po::bool_switch(), "POM only: build a single hemisphere (upper polar+equatorial PMTs) with a flat plastic cap sealing the equator, instead of the full module, to compare against a half-module prototype measurement")
    ("single_pmt_gelpad", po::bool_switch(), "detector_type=1 (Single PMT) only: glue a gelpad cone onto the PMT's photocathode-facing tip, to compare against a bare-PMT-plus-gelpad prototype measurement")
	("detector_type", po::value<G4int>()->default_value(2), "module type [custom = 0, Single PMT = 1, mDOM = 2, DOM = 3, LOM16 = 4, LOM18 = 5, DEGG = 6, pDOM (HQE deepcore) = 7,POM=8 (P-ONE Optical Module)]")
...
void OMSim::startVisualisation()
{
    ...
    G4int environment = OMSimCommandArgsTable::getInstance().get<G4int>("environment");
    
    //uiInterface.applyCommand("/vis/viewer/set/background", Tools::getWorldBackground(environment));
    uiEx->SessionStart();
    delete uiEx;
}
